...
async function searchJobs(req, res) {
  try {
    ...
    const [jobs, total] = await Promise.all([query.exec(), Job.countDocuments(filter)]);
    const enhanced = jobs.map(j => ({ ...j.toObject(), matchScore: q ? (j.score || 0) : null }));
...
async function applyJob(req, res) {
  try {
    ...
    if (!job.meta.appliedUsers.some(id => id.toString() === userId.toString())) {
